...
public class CheckOldCustomFormTest extends BaseTestNGWebDriver {
	...
	public HealthFormPage checkOldCustomForm(WebDriver driver, TestcasesData testcasesData, String formName)
			throws Exception {
		...
		/*
		 * Not very elegant way of changing URL the method uses If someone sets
		 * the URL in the object before calling this method then the set URL is
		 * be used otherwise (the default way) the URL from
		 * testcasesData.geturl() is used
		 */
		...
		log("step 3: Click on CustomForm");
		...
		Thread.sleep(8000);
		/*
		 * assertEquals(verifyTextPresent(driver, "completed 2 of 3 pages"),
		 * true, "Partialy completed form not saved correctly");
		 */
	...
	public void checkDeletedPages(WebDriver driver, HealthFormPage page, String formName) throws Exception {
		...
		Thread.sleep(8000);
		verifyEquals(verifyTextPresent(driver, "completed 0 of 1 pages"), true,
				"Partialy completed form after deleted pages not displayed correctly");
		page.selectOldCustomForm(formName);
		verifyEquals(verifyTextPresent(driver, "First Name"), true,
				"Demographic information is not present in form on Portal");
	}
