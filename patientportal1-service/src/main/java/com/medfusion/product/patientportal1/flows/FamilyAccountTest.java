...
public class FamilyAccountTest extends BaseTestNGWebDriver {
	...
	public void FamilyAccount(WebDriver driver, TestcasesData data) throws Exception {
		...
		pMyPatientPage = pCreatefamilymemberPage.FamilyMemberdetails(testcasesData.getFamilyUN(), testcasesData.getFamilyPW(), testcasesData.getRelationship());

		log("step 8:Click on unlink link");
		pMyPatientPage = pCreatefamilymemberPage.Verifymember();

		/*
		 * log("Delete the account created"); log("step 8:Click on myaccountLink on MyPatientPage"); pMyAccountPage = pMyPatientPage.clickMyAccountLink();
		 * 
		 * log("step 9:Click on FamilyLink  on MyAccountPage"); pPatientsOnThisAccountPage = pMyAccountPage.clickfamilyLink();
		 * 
		 * // log("step 10:Assert the dependent Name"); // pPatientsOnThisAccountPage .verifyName(testcasesData.getpatientFirstName(), //
		 * testcasesData.getpatientLastName());
		 * 
		 * log( "step 11:Click on UnlinkDependentLink  on PatientsOnThisAccountPage " ); UnlinkPatientFromFamilyPage pUnlinkPatientFromFamilyPage =
		 * pPatientsOnThisAccountPage .clickUnlinkDependentLink();
		 * 
		 * log( " step 12:Give ur email address and confirm it UnlinkPatientFromFamilyPage " ); pPatientsOnThisAccountPage = pUnlinkPatientFromFamilyPage
		 * .unLinkFamilyMember(testcasesData.getEmail());
		 * 
		 * log(" step 13:unlinked Dependent Message "); pPatientsOnThisAccountPage.verifyunlinkedDependentMessage( testcasesData.getFirstName(),
