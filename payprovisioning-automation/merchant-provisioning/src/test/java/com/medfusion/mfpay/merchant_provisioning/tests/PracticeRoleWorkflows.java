...
public class PracticeRoleWorkflows extends BaseRest{
    ...
    public void testSetPracticeRoleToMerchantUser() throws Throwable {
        ...
        Response response = usersdetails.createPracticeUser(getusers, testData.getProperty("staff.username"), testData.getProperty("practice.role"));

        Validations validations = new Validations();
        JsonPath jsonpath = new JsonPath(response.asString());
        validations.validatePracticeRoles(jsonpath, testData.getProperty("practice.staffid"), testData.getProperty("practice.id"), Arrays.asList(testData.getProperty("practice.role")));
    }

    @Test(dataProvider = "practice_role_test", dataProviderClass = MPTestData.class, enabled = true)
    public void testSetPracticeRoleToMerchantUserInvalidData(String practiceStaffId, String practiceId,
                                                             String username, String practiceRole) throws IOException {
