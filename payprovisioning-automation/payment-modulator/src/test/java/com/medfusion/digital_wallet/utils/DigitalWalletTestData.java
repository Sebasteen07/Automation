...
//				//Merchant with incorrect Client ID & Client Secret
    ...
    @DataProvider(name = "valid_cards")
    public static Object[][] dataProviderValidCardInput() throws Exception {
        testData = new PropertyFileLoader();
        String token = GatewayProxyUtils.getTokenForCustomer();
        return new Object[][]{

                {testData.getProperty("deafult.payment.method.card"), testData.getProperty("account.alias"),
                        testData.getProperty("instamed.card.expiration.number"), testData.getProperty("card.number4"),
                        testData.getProperty("type4"), testData.getProperty("cvv")},

                {testData.getProperty("deafult.payment.method.card"), testData.getProperty("account.alias"),
                        testData.getProperty("instamed.card.expiration.number"),
                        testData.getProperty("card.number1"), testData.getProperty("type1"), testData.getProperty("cvv")},

                {" ", testData.getProperty("account.alias"), testData.getProperty("instamed.card.expiration.number"),
                        testData.getProperty("instamed.card.number.mc"), testData.getProperty("type3"), testData.getProperty("cvv")},

                {"", testData.getProperty("account.alias"), testData.getProperty("instamed.card.expiration.number"),
                        testData.getProperty("card.number2"), testData.getProperty("type2"), testData.getProperty("cvv.amex")}
        };
    }

    @DataProvider(name = "instamed_different_patients_adding_card")
    public static Object[][] dataProviderDifferentPatientIds() throws Exception {
        testData = new PropertyFileLoader();

        return new Object[][]{
                //Duplicate Scenario
                {testData.getProperty("patient.id"), 400, "Bad Request", "The card already exists in the wallet"},

                //Wallet does not exists scenario
                {IHGUtil.createRandomNumericString(36), 404, "Not Found", "The patient does not have associated wallet"},

        };
    }

    @DataProvider(name = "wallet_card_invalid_data")
    public static Object[][] dataProviderAddCardToWallet() throws Exception {
        testData = new PropertyFileLoader();
        String token = GatewayProxyUtils.getTokenForCustomer();
        return new Object[][]{
                {testData.getProperty("instamed.mmid"), testData.getProperty("default.payment.method.echeck"),
    ...
    @DataProvider(name = "sale_invalid_data")
