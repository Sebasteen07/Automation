...
public class IntegrationEngineTest {
	...
	static {
		healthHistoryQuestionFields.put(Integer.toString(HEALTHHISTORYQUESTIONID_MEMBERID), Integer.toString(SystemConfigProperties.WSFM_MEMBERID1));
	}
	...
	private static Map<String, String> getMedicationQuestionFields(Integer memberId, String medicationId, String medicationName, String medicationStatus,
			String medicationDosage, String medicationDosageUnit, String medicationFrequency) {
		Map<String, String> medicationQuestionFields = new HashMap<String, String>();

		medicationQuestionFields.put(Integer.toString(MEDICATIONQUESTIONID_MEMBERID), Integer.toString(memberId));
		if (medicationId != null)
			medicationQuestionFields.put(Integer.toString(MEDICATIONQUESTIONID_MEDICATIONID), medicationId);
		medicationQuestionFields.put(Integer.toString(MEDICATIONQUESTIONID_MEDICATIONNAME), medicationName);
		medicationQuestionFields.put(Integer.toString(MEDICATIONQUESTIONID_MEDICATIONSTATUS), medicationStatus);
		medicationQuestionFields.put(Integer.toString(MEDICATIONQUESTIONID_MEDICATIONDOSAGE), medicationDosage);
		medicationQuestionFields.put(Integer.toString(MEDICATIONQUESTIONID_MEDICATIONDOSAGEUNIT), medicationDosageUnit);
		medicationQuestionFields.put(Integer.toString(MEDICATIONQUESTIONID_MEDICATIONFREQUENCY), medicationFrequency);
	 ...
	 */
	private static net.medfusion.integrations.webservices.intengine.objects.Field ieField(String fieldName, Object fieldValue, String fieldDataType) {
		return new net.medfusion.integrations.webservices.intengine.objects.Field(fieldName, fieldValue, fieldDataType);
	}

	private static ChangeGroup createChangeGroup(Integer questionGroupId, Map<String, String> questionFields) {
		ChangeGroup changeGroup = new ChangeGroup();

		List<Field> fieldList = new ArrayList<Field>();
		fieldList.add(ieField(WebServiceConstants.FIELD_CHQG_QUESTIONGROUPID, questionGroupId, WebServiceConstants.FIELD_DATATYPE_INT));

		ChangeQuestion[] changeQuestions = createChangeQuestions(questionFields);
		fieldList.add(ieField(WebServiceConstants.FIELD_CHQG_CHANGEQUESTIONS, changeQuestions, WebServiceConstants.FIELD_DATATYPE_ANY));
		...
		theFields.setFields((net.medfusion.integrations.webservices.intengine.objects.Field[]) fieldList
				.toArray(new net.medfusion.integrations.webservices.intengine.objects.Field[fieldList.size()]));
	...
	/**
	 * Create array of ChangeQuestion objects - Add ChangeQuestion for each Field
	 * 
	 * @param questionFields
	 * @return ChangeQuestion[]
	 */
	private static ChangeQuestion[] createChangeQuestions(Map<String, String> questionFields) {
		ChangeQuestion[] changeQuestionArray = new ChangeQuestion[questionFields.size()];
		...
		for (Iterator<String> iterator = keys.iterator(); iterator.hasNext();) {
			...
			List<Field> fieldList = new ArrayList<Field>();
			fieldList.add(ieField(WebServiceConstants.FIELD_CHQ_QUESTIONID, Integer.valueOf(setKey), WebServiceConstants.FIELD_DATATYPE_INT));
			fieldList.add(ieField(WebServiceConstants.FIELD_CHQ_VALUE, setValue, WebServiceConstants.FIELD_DATATYPE_STRING));
	...
	public void oneTimeSetUp() throws Exception {
		...
		ieLocator.setintengineEndpointAddress(SystemConfigProperties.intEngineURL);
	...
	public void testChangeServices() {
		try {
			...
			Change[] chResults = ieService.getOutboundChanges(WebServiceConstants.FIELD_INT_NONE, WebServiceConstants.FIELD_INT_NONE);
			WebserviceUtils.dumpResults(chResults);

			/*
			 * if (chResults != null && chResults.length > 0) { System.out.println("Set the first change to Error"); Change ch = chResults[0]; Integer changeId =
			 * ch.getFieldValueInt(WebServiceConstants.FIELD_CHANGE_ID); service.setChangeAsError(changeId.intValue(), "This is a sample error message"); }
			 * 
			 * if (chResults != null && chResults.length > 1) { System.out.println("Set the second change to Processed"); Change ch = chResults[1]; Integer changeId =
			 * ch.getFieldValueInt(WebServiceConstants.FIELD_CHANGE_ID); int[] changeIds = new int[1]; changeIds[0] = changeId.intValue();
	...
	public void testCreateOrUpdateInboundMedicationServices() {
		try {
			...
			List<Field> fieldList = new ArrayList<Field>();
			fieldList.add(ieField(WebServiceConstants.FIELD_CHANGE_INTEGRATIONID, SystemConfigProperties.WSFM_INTEGRATIONID, WebServiceConstants.FIELD_DATATYPE_INT));
			fieldList
					.add(ieField(WebServiceConstants.FIELD_CHANGE_TYPE, WebServiceConstants.FIELD_CHANGE_TYPE_INBOUNDADD, WebServiceConstants.FIELD_DATATYPE_STRING));
			...
			fieldList.add(ieField(WebServiceConstants.FIELD_CHANGE_PRACTICEID, SystemConfigProperties.WSFM_PRACTICEID, WebServiceConstants.FIELD_DATATYPE_INT));

			ChangeGroup[] changeGroups1 = new ChangeGroup[2];
			changeGroups1[0] = createChangeGroup(HEALTHHISTORYQUESTIONGROUPID, healthHistoryQuestionFields);
			changeGroups1[1] = createChangeGroup(MEDICATIONQUESTIONGROUPID,
					getMedicationQuestionFields(SystemConfigProperties.WSFM_MEMBERID1, null, "Vicodin", "Active", "50", "mg", "After meals"));
			fieldList.add(ieField(WebServiceConstants.FIELD_CHANGE_CHANGEQUESTIONGROUPS, changeGroups1, WebServiceConstants.FIELD_DATATYPE_ANY));
			...
			System.out.println("Let's Create some Inbound Medication Changes for Integration Id - " + SystemConfigProperties.WSFM_INTEGRATIONID);
			WebServiceReturnComposite[] wsrc = ieService.createOrUpdateInboundChanges(changes, SystemConfigProperties.EXTERNALSYSTEM_TESTEMR);
