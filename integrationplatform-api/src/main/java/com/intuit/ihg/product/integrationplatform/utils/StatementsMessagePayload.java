...
public class StatementsMessagePayload {
	...
	public String getStatementsMessagePayload(StatementEventData testData) throws ParseException {
		...
		try {
			...
			Node EncounterChargeInsuranceAdjustmentsTotal = doc
					.createElement("EncounterChargeInsuranceAdjustmentsTotal");
	...
	public String getStatementsMessageV3Payload(StatementEventData testData) throws ParseException {
		...
		try {
			icBuilder = icFactory.newDocumentBuilder();
			Document doc = icBuilder.newDocument();
			String schema = "http://schema.medfusion.com/statements/v3";
			Thread.sleep(500);
			Element mainRootElement = doc.createElementNS(schema, "ns2:Statements");
	...
	public String getStatementsMessageV3MiniPayload(StatementEventData testData, String method) throws ParseException {
		DocumentBuilderFactory icFactory = DocumentBuilderFactory.newInstance();
		DocumentBuilder icBuilder;
		...
		try {
			...
			Node StatementFormat = doc.createElement("StatementFormat");
			StatementFormat.appendChild(doc.createTextNode("FORMAT1"));
			Statement.appendChild(StatementFormat);
			...
			workingDir = workingDir + testData.StatementPdf_Detail;

			StatementValueEncoded = ExternalFileReader.readFromFile(workingDir);
			System.out.println("Working directory " + workingDir);

			Node StatementPdfDetail = doc.createElement("StatementPdfDetail");
			StatementPdfDetail.appendChild(doc.createTextNode(StatementValueEncoded));
			Statement.appendChild(StatementPdfDetail);
