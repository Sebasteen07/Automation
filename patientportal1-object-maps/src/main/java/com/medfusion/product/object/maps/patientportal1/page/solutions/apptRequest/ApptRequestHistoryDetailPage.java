...
public class ApptRequestHistoryDetailPage extends BasePageObject {
	...
	/**
	 * Clicks the 'View As PDF' link from the Appt Request History detail page. Will attempt to return the text in the PDF by copying all the text from the PDF
	 * for validation. The PDF contained in the page via the HTML 'embed' tag.
	 * 
	...
	public String clickViewAsPdfLink() throws InterruptedException {
		 ...
		 * 
		 * Set<String> windowHandles = driver.getWindowHandles(); for (String handle : windowHandles) { driver.switchTo().window(handle);
		 * if(!currentWindowHandle.equalsIgnoreCase(handle)) { driver.switchTo().window(handle); } }
		 */

		/*
		 * WebElement body = driver.findElement(By.tagName("body")); body.sendKeys(Keys.chord(Keys.CONTROL, "a")); body.sendKeys(Keys.chord(Keys.CONTROL, "c"));
		 */

		/*
		 * TextTransfer clipboard = new TextTransfer(); return clipboard.getClipboardContents();
		 */
	}
