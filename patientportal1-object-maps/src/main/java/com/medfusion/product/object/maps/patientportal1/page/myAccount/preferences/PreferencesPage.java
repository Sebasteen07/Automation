...
public class PreferencesPage extends BasePageObject {
    ...
    public boolean isProviderPreferred(String preferredProvider) {
        ...
        for (WebElement row : allRows) {
            List<WebElement> cells = row.findElements(By.tagName("td"));
            for (WebElement cell : cells) {
                log("Searching for provider: " + preferredProvider + ", Found preferred provider: "
     ...
     * 
     * @param provider
     *            the provider's name as it is displayed in the select box
     * @throws Exception
     *             An exception be thrown if the select box can't be found or if the
