package com.intuit.ihg.product.object.maps.portal.page.myAccount.preferences;
...
public class PreferencesPage extends BasePageObject {
	...
	/**
	 * Searches to see if the given provider is already listed as a preferred provider.
	 * If so, the 'Remove' link will be clicked to remove the given provider as
	...
	public boolean isProviderPreferred(String preferredProvider) {
		...
		for (WebElement row : allRows) {
			List<WebElement> cells = row.findElements(By.tagName("td"));
			for (WebElement cell : cells) {
				log("Searching for provider: " + preferredProvider +
						", Found preferred provider: " + cell.getText().toString());
	 ...
	 * @throws Exception An exception be thrown if the select box can't be found or if the
	 * provider's name can't be found as an option.
	 */
