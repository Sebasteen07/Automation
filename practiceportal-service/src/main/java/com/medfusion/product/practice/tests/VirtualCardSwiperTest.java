...
public class VirtualCardSwiperTest extends BaseTestNGWebDriver {
	...
	public void virtualCardSwipeTest(WebDriver driver, PropertyFileLoader testData, String cardType)
		...
		if (cardType.equalsIgnoreCase(PracticeConstants.CARD_TYPE_VISA)) {
			...
			practiceHome = practiceLogin.login(testData.getProperty("paypal.doctor"),
					EncryptionUtils.decrypt(testData.getProperty("paypal.password")));
		}
		...
		if (swipeString.isEmpty()) {
			virtualCardSwiper.addCreditCardInfo(PracticeConstants.CARD_NAME, PracticeConstants.CARD_NUMBER,
					...
					PracticeConstants.CVV, PracticeConstants.ZIP, PracticeConstants.PATIENT_ACCOUNT,
					PracticeConstants.PATIENT_NAME, PracticeConstants.COMMENT);
		} else {
			virtualCardSwiper.addCreditCardMandatoryInfo(PracticeConstants.CARD_NAME,
					PracticeConstants.CARD_NUM_MASTERCARD, PracticeConstants.CARD_TYPE_MASTERCARD,
					PracticeConstants.EXP_MONTH, PracticeConstants.EXP_YEAR, amount, PracticeConstants.ZIP,
					PracticeConstants.SWIPE_STRING_MASTERCARD, PracticeConstants.PATIENT_ACCOUNT,
					PracticeConstants.PATIENT_NAME);
		}
	...
	public String virtualCardSwipeTest(WebDriver driver, PropertyFileLoader testData) throws Exception {
		...
		logStep("Enter credentials and login");
		PracticeHomePage practiceHome = practiceLogin.login(testData.getProperty("doctor.login"),
				EncryptionUtils.decrypt(testData.getProperty("doctor.password")));
