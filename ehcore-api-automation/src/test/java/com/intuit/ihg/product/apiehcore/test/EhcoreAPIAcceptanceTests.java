...
public class EhcoreAPIAcceptanceTests extends BaseTestNGWebDriver{
	...
	(),TrackingEnumHolder.OBJECTSTORE_NODE_TYPE.DELTA.toString());
		...
		());
			 ...
			 * @User Story ID in Rally : US6779
			 * @StepsToReproduce:	
			 * Test - check the status of 'XML Validation' in Tracking DB for CCDImportMessage
			 * Expected Result :200 OK
			 * @throws Exception 
			 */
		...
		(),EhcoreAPIConstants.AS_RAWRETRIEVAL));
			...
			public void testDCPublish2ConnectHubActivity() throws Exception {
				...
				ProcessingResponse response = EhcoreAPIUtil.sendCCDExportMessage(testData.getPatientID(),CCDExportConstants.AS_CCD_EXPORT);
				List<Message> msgDetailsNew = new ArrayList<Message>();
				msgDetailsNew = EhcoreAPIUtil.verifyExpectedMessageProcStatus(0,response.getMessageId(),TrackingEnumHolder.MESSAGE_STATUS.ERROR.toString
		...
		(),EhcoreAPIConstants.AS_RAWPERSIST));
			...
			/*	
				...
				EHCoreTestUtil.verifyExpectedMessageProcStatus(0,response.getDataJobId(),TrackingEnumHolder.MESSAGE_STATUS.COMPLETED.toString(),EHCoreTestConsts.EH_REST_API_Consts.CCDExport);
				EHCoreTestUtil.verifyExpectedDataJobProcStatus(response.getDataJobId(),TrackingEnumHolder.DATAJOB_STATUS.DONE.toString());
				*/
			...
			public void testRawMessageRetrieveForCCDExport() throws Exception {
				...
				verifyTrue(EhcoreTrackingDBUtils.isActivityStatusCompleted(msgDetails.get(0).getQid().toString(),TrackingEnumHolder.ACTIVITY_TYPE.RETRIEVE.toString(),EhcoreAPIConstants.RAWRETRIEVAL)); 
...
(),TrackingEnumHolder.ACTIVITY_TYPE.ENRICH.toString(),EhcoreAPIConstants.CHANGEDETECTION));
	...
	* @throws Exception 
	...
	public void testCDWithUpdate_CCCD() throws Exception {
		...
		cCCDmsgDetails = EhcoreAPIUtil.verifyExpectedMessageProcStatus(0,dj.getDataJobId(),TrackingEnumHolder.MESSAGE_STATUS.COMPLETED.toString(),CCDIMPORT_MSG_TYPE);
		assertTrue(EhcoreTrackingDBUtils.isActivityStatusCompleted(cCCDmsgDetails.get(0).getQid().toString(),TrackingEnumHolder.ACTIVITY_TYPE.ENRICH.toString(),EhcoreAPIConstants.CHANGEDETECTION));

		//Send the same message with updated details(updated Allery section)
		EhcoreAPIUtil.sendUpdatedMessage(dj.getDataJobId(),cCCDmsgDetails.get(0).getSubjectId(),EhcoreAPIConstants.updateC_CCD);
		cCCDmsgDetails = EhcoreAPIUtil.verifyExpectedMessageProcStatus(1,dj.getDataJobId(),TrackingEnumHolder.MESSAGE_STATUS.COMPLETED.toString(),CCDIMPORT_MSG_TYPE);//cCCDmsgDetails.get(0).getQid().toString());
		assertTrue(EhcoreTrackingDBUtils.isActivityStatusCompleted(cCCDmsgDetails.get(1).getQid().toString(),TrackingEnumHolder.ACTIVITY_TYPE.ENRICH.toString(),EhcoreAPIConstants.CHANGEDETECTION));
		
		/**
		 * To get the Added CDM List we need to check the second message in this subject_id.
		 */
		cCCDmsgDetails = EhcoreAPIUtil.verifyExpectedMessageProcStatus(1,dj.getDataJobId(),TrackingEnumHolder.MESSAGE_STATUS.COMPLETED.toString(),cCCDmsgDetails.get(0).getQid().toString());
		//Get the Actual CDM Message and compare with expected using message_guid in MongoDB
