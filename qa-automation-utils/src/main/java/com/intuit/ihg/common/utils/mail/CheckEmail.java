...
public class CheckEmail {
	...
	public static boolean validateForgotUserID(Gmail gmail, Date startEmailSearchDate, String recipientEmail, String subjectSubString, String userName)
		...
		for (int i = 0; i < 6; i++) {
			...
			if (messages.length > 0) {
				for (Message m : messages) {
					...
					msgCount++;
					Log4jUtil.log("Checking email " + msgCount + " (received at " + dateFormat.format(m.getReceivedDate()) + ") of " + messages.length);
				}
			} else {
				Thread.sleep(10000);
			}
