...
public class JalapenoNewCustomHealthFormPage extends MedfusionPage {

	@FindBy(how = How.ID, using = "continueWelcomePageButton")
	private WebElement continueWelcomePageButton;

	@FindBy(how = How.CSS, using = ".save")
	private WebElement saveAndFinishButton;

	@FindBy(how = How.ID, using = "prevPageButton")
	private WebElement prevPageButton;

	@FindBy(how = How.ID, using = "nextPageButton")
	private WebElement saveAndContinueButton;

	@FindBy(how = How.CSS, using = ".submit")
	private WebElement submitFormButton;

	@FindBy(how = How.ID, using = "chooseSectionMenu")
	private WebElement chooseSectionMenuSelect;

	@FindBy(how = How.ID, using = "section0")
	private WebElement welcomePageDiv;

	@FindBy(how = How.ID, using = "section1")
	private WebElement section1Div;

	@FindBy(how = How.ID, using = "section2")
	private WebElement section2Div;

	@FindBy(how = How.ID, using = "section3")
	...
	public JalapenoNewCustomHealthFormPage(WebDriver driver) {
		super(driver);
	...
	public List<WebElement> getVisibleQuestions() {
		return driver.findElements(By.xpath(
				String.format(QuestionUtils.QUESTIONS_BASE_XPATH, currentSection) + "[(count(./li)=1 or ./label)]"));
	}
	...
	public WebElement getVisibleQuestion(int questionOrdinal) {
		return driver.findElement(By.xpath(String.format(QuestionUtils.QUESTIONS_BASE_XPATH, currentSection)
				+ "[(count(./li)=1 or ./label)])[" + questionOrdinal + "]"));
	}

	public boolean isHeadingDisplayed(String headingTitle) {
		List<WebElement> headings = driver.findElements(
				By.xpath("//form[" + currentSection + "]//fieldset//h3[not(parent::*[contains(@class,'hiddenFUP')])]"));
	...
	public boolean isTextDisplayed(String text) {
		List<WebElement> textDivs = driver
				.findElements(By.xpath(String.format(QuestionUtils.QUESTIONS_BASE_XPATH, currentSection) + "/div"));
	...
	/**
	 * 
	 * @param questionOfSelectType
	 *            WebElement &lt;ul&gt; object
	 * @param answerIndex
	 *            index from 1
	 */
	...
	public void consentAllVisibleStatements(String consentString) {
		for (WebElement consentInput : driver.findElements(By.xpath(
				String.format(QuestionUtils.QUESTIONS_BASE_XPATH, currentSection) + "[count(./li)=2]/li/input"))) {
			consentInput.sendKeys(consentString);
		}
	}

	/**
	 * clears all inputs of specified section, in case of radio buttons it
	 * always selects the last option
	 */
	public void clearAllInputs() {
		driver.manage().timeouts().implicitlyWait(100, TimeUnit.MILLISECONDS);
		for (WebElement fupsel : driver
				.findElements(By.xpath(String.format(QuestionUtils.QUESTIONS_BASE_XPATH, currentSection)
						+ FUP_QUESTIONS_FILTER + "/label/input"))) {
		...
		for (WebElement fuptxt : driver
				.findElements(By.xpath(String.format(QuestionUtils.QUESTIONS_BASE_XPATH, currentSection)
						+ FUP_QUESTIONS_FILTER + "/li/*[self::input|self::textarea]"))) {
			fuptxt.clear();
		}
		for (WebElement sel : driver
				.findElements(By.xpath(String.format(QuestionUtils.QUESTIONS_BASE_XPATH, currentSection)
						+ TOPLVL_QUESTIONS_FILTER + "/label/input"))) {
		...
		for (WebElement txt : driver
				.findElements(By.xpath(String.format(QuestionUtils.QUESTIONS_BASE_XPATH, currentSection)
						+ TOPLVL_QUESTIONS_FILTER + "/li/*[self::input|self::textarea]"))) {
			txt.clear();
		}
