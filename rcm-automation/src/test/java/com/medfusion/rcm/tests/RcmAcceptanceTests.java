...
public class RcmAcceptanceTests extends BaseTestNGWebDriver {
    ...
    public void testSendStmtVerifyNotificationsMessagesBalance() throws Exception {
        ...
        log("Post eStatement");
        util.postStatementToPatient(testData.getRcmStatementRest() + testData.getPracticeId() + "/patients/"
                + testData.getMemberId() + "/statements", IHGUtil.getEnvironmentType().toString());

        log("Check email notification and URL");
        String box = testData.getEmail().split("@")[0];
        assertTrue(mail.catchNewMessageCheckLinkUrl(box, "Your patient eStatement is now available",
                "Visit our website", testData.getUrl(), 50));
    ...
    public void testSendStmtE2ESftpGWMirth() throws Exception {
        ...
        PropertyFileLoader testData = new PropertyFileLoader();
        URL url = ClassLoader
                .getSystemResource("testfiles/" + IHGUtil.getEnvironmentType().toString() + "/" + "gwayestAUTO.002");
        ...
        String box = testData.getEmail().split("@")[0];
        assertTrue(mail.catchNewMessageCheckLinkUrl(box, "Your patient eStatement is now available",
                "Visit our website", testData.getUrl(), 108));
    ...
    public void testSendDynamicPaperFormat1() throws Exception {
        ...
        // TODO duplicated code - on line 283
        ...
        String endpoint = testData.getRcmStatementRest() + testData.getPracticeId() + "/patients/" + newPat.memberId
                + "/statements";
        newPat.billingAccountNumber = postModifiedStatementToPatient("statementDF1.txt", endpoint,
                IHGUtil.getEnvironmentType().toString(), newPat.practicePatientId, newPat.balance, "PF1");
        assertFalse(newPat.billingAccountNumber == -1);
        log("Statement was successfuly posted to user rest, to the following billing account number: "
        ...
        assertTrue(getBillingAccountInfoComparePatientBalance(testData.getRcmBillingAccountRest(),
                Integer.toString(newPat.billingAccountNumber), testData.getDoctorBase64AuthString(), newPat.balance));
        log("Checks out!");
    }
    /*
    ...
    // This method is specifically intended for new patient test, does not guarantee the found balance is found in
    // summary and not old statements (but new patients will only have one statement)
    protected boolean getBillingAccountInfoComparePatientBalance(String rcmBillingAccountRest,
            String billingAccountNumber, String staffAuthString, String balanceToFind) {
    ...
    protected void sendDynamicByFileAndPreference(String filename, String idPrefix, int preference) throws Exception {
        ...
        String endpoint = testData.getRcmStatementRest() + testData.getPracticeId() + "/patients/" + newPat.memberId
                + "/statements";
        newPat.billingAccountNumber = postModifiedStatementToPatient(filename, endpoint,
                IHGUtil.getEnvironmentType().toString(), newPat.practicePatientId, newPat.balance, idPrefix);
        assertFalse(newPat.billingAccountNumber == -1);
        log("Statement was successfuly posted to user rest, to the following billing account number: "
                + newPat.billingAccountNumber);
        ...
        // check presence of badge (electronic preference &&(last payment date < newest statement date || (no payments
        // present && statement arrived)))
        if (preference == 2) {
            log("No payments present, expect a badge with balance on Pay Bills");
            String badge = driver
                    .findElement(By.xpath("//a[@id='feature_bill_pay']/span[@class='badge amountDue ng-binding']"))
                    .getText();
    ...
    protected PatientInfo activateNewPatientPrepareForStatement(WebDriver driver, PropertyFileLoader testData,
            int deliveryPref) throws Exception {
        log(this.getClass().getName());
        ...
        JalapenoIdentifyPatientBeforeActivationPage jalapenoIdentifyPatientBeforeActivationPage;
        JalapenoHomePage jalapenoHomePage;
        try {
            log("Finishing of patient activation: step 1 - verifying identity");
            jalapenoIdentifyPatientBeforeActivationPage = new JalapenoIdentifyPatientBeforeActivationPage(driver,
                    newPat.unlockLink);
            ...
            JalapenoPatientCreateSecurityDetailsPage jalapenoPatientCreateSecurityDetailsPage = jalapenoIdentifyPatientBeforeActivationPage
                    .fillInPatientDataAndSubmitForm(jalapenoPatient);
            checkAlert(driver);

            log("Finishing of patient activation: step 2 - filling patient data");
            jalapenoHomePage = jalapenoPatientCreateSecurityDetailsPage.fillInPatientActivationWithDeliveryPreference(
                    newPat.firstName, testData.getPassword(), testData.getSecretQuestion(), testData.getSecretAnswer(),
    ...
    protected int postModifiedStatementToPatient(String filename, String endpoint, String env, String practicePatientId,
            String patientBalance, String prefix) throws Exception {
        ...
        String payload;
        try {
            System.out.println("Building statement for " + env + " , setting patient to " + practicePatientId
                    + ", generated balance " + patientBalance + " , new account number " + newBillingNumber);
