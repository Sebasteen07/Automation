...
public class PSS2PatientPortalAcceptanceTests04 extends BaseTestNGWebDriver {
	...
	public void testAppointmentStackingNG() throws Exception {
		...
		homePage.btnStartSchedClick();
		String thirdPatientTime = pssPatientUtils.bookLTB(homePage, testData, driver, locationName, appType,
				providerName);
	...
	public void testAppointmentStackingGE() throws Exception {
		...
		String firstNameP3 = propertyData.getProperty("stacking.p3.firstname.ge");
		String lastNameP3 = propertyData.getProperty("stacking.p3.lastname.ge");
		String dobP3 = propertyData.getProperty("stacking.p3.dob.ge");
		String genderP3 = propertyData.getProperty("stacking.p3.gender.ge");
	...
	@Test(enabled = true, groups = { "AcceptanceTests" }, retryAnalyzer = RetryAnalyzer.class)
	public void testBusinessHourWithLocationTimezoneNG() throws Exception {
		PSSPropertyFileLoader propertyData = new PSSPropertyFileLoader();
		Appointment testData = new Appointment();
		AdminUser adminuser = new AdminUser();
		propertyData.setAdminNG(adminuser);
		...
		log("TimeZone- " + locationTimeZone);
		testData.setCurrentTimeZone(locationTimeZone);
		String currentTime = psspatientUtils.currentTimeWithTimeZone(locationTimeZone);
		...
		AppointmentDateTime aptDateTime = provider.getProviderandClick(testData.getProvider());
		String date = aptDateTime.selectDate(testData.getIsNextDayBooking());
		logStep("Date selected is for Appointment" + date);
		log("Next date is    " + psspatientUtils.numDate(testData));
		if (psspatientUtils.timeDifferance(testData, endTime) < 0) {
			log("Time Diff is  less Than 0 print Next date ");
			assertEquals(date, psspatientUtils.numDate(testData));
	...
	@Test(enabled = true, groups = { "AcceptanceTests" }, retryAnalyzer = RetryAnalyzer.class)
	public void testBusinessHourWithLocationTimezoneGW() throws Exception {
		PSSPropertyFileLoader propertyData = new PSSPropertyFileLoader();
		Appointment testData = new Appointment();
		AdminUser adminuser = new AdminUser();
		propertyData.setAdminGW(adminuser);
	...
	@Test(enabled = true, groups = { "AcceptanceTests" }, retryAnalyzer = RetryAnalyzer.class)
	public void testBusinessHourWithLocationTimezoneGE() throws Exception {
		PSSPropertyFileLoader propertyData = new PSSPropertyFileLoader();
		Appointment testData = new Appointment();
		AdminUser adminuser = new AdminUser();
		propertyData.setAdminGE(adminuser);
	...
	public void testLinkGenWithSpecialityNG() throws Exception {
		...
		logStep("Patient Matching By Using Adapter Modulator");
		Response response = postAPIRequestAM.patientInfoPost(practiceId, payloadAM.patientInfoWithOptionalGW());
		apv.responseCodeValidation(response, 200);
		...
		String expectedSpeciality = propertyData.getProperty("speciality.name04");
		String actualSpeciality = speciality.selectSpeciality1(expectedSpeciality);
		assertNotEquals(expectedSpeciality, actualSpeciality);
		Response resetSpeciality = postAPIRequestAM.specialitySave(practiceId,
				payloadAM01.saveResetSpecialityNG(specialityId));
	...
	public void testLinkGenWithSpecialityGE() throws Exception {
		...
		apv.responseCodeValidation(spe, 200);
		logStep("Patient Matching By Using Adapter Modulator");
		Response response = postAPIRequestAM.patientInfoPost(practiceId, payloadAM.patientInfoWithOptionalGE());
		apv.responseCodeValidation(response, 200);
		log("link is   " + testData.getLinkProviderURL());
		DismissPage dismissPage = new DismissPage(driver, testData.getLinkProviderURL());
		Thread.sleep(1000);
