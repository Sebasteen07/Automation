...
public class IntegrationPlatformAcceptanceTests extends BaseTestNGWebDriver {
	 ...
	 * = RetryAnalyzer.class) public void testGetAppointmentRequest() throws
	 * Exception {
	 * 
	 ...
	 * AppointmentTestData(aptData); Long timestamp =
	 ...
	 * 
	 * log("Step 2: LogIn"); PortalLoginPage loginPage = new
	 * PortalLoginPage(driver, testData.getUrl());
	 ...
	 * 
	 * //get only messages from last day in epoch time to avoid transferring lot
	 * of data Long since = timestamp / 1000L - 60 * 60 * 24;
	 * 
	...
	public void testE2EAppointmentRequest() throws Exception {
		...
		MyPatientPage myPatientPage = loginPage.login(testData.getUserName(),
				testData.getPassword());

		log("Step 3: Click on Appointment Button on My Patient Page");
		AppointmentRequestStep1Page apptRequestStep1 = myPatientPage
				.clickAppointmentRequestTab();

		log("Step 4: Complete Appointment Request Step1 Page  ");
		AppointmentRequestStep2Page apptRequestStep2 = apptRequestStep1
				.requestAppointment(null, null, testData.getPreferredDoctor(),
						null);

		log("Step 5: Complete Appointment Request Step2 Page  ");
		AppointmentRequestStep3Page apptRequestStep3 = apptRequestStep2
				.fillInForm(PortalConstants.PreferredTimeFrame,
						PortalConstants.PreferredDay,
						PortalConstants.ChoosePreferredTime,
						PortalConstants.ApptReason,
						PortalConstants.WhichIsMoreImportant,
						testData.getPhoneNumber());

		log("Getting Appointment reason ");
		long time = apptRequestStep2.getCreatedTs();
		String reason = PortalConstants.ApptReason.toString() + " "
				+ String.valueOf(time);
		...
		log("Step 6: Complete Appointment Request Step3 Page  ");
		AppointmentRequestStep4Page apptRequestStep4 = apptRequestStep3
				.clickSubmit();
		...
		PortalLoginPage ploginPage = new PortalLoginPage(driver,
				emailMessageLink);
	...
	public void testAMDCAskQuestion() throws Exception {
		...
		AskAStaffStep2Page askStaff2 = askStaff1
				.askQuestion("Test",
						"This is generated from the AMDCAskQuestion automation test case.");
		...
		verifyTrue(
				aasHistory.isAskAStaffOnHistoryPage(Long.toString(askStaff1
						.getCreatedTimeStamp())),
				"Expected to see a subject containing "
						+ askStaff1.getCreatedTimeStamp()
		...
		log("Step 10: Checking validity of the response xml");
		RestUtils.isQuestionResponseXMLValid(testData.getResponsePath(),
				askStaff1.getCreatedTimeStamp());
	}
	...
	public void testPIDCPatientUpdate() throws Exception {
		...
		PortalLoginPage loginpage = new PortalLoginPage(driver,
				testData.getUrl());
		MyPatientPage pMyPatientPage = loginpage.login(testData.getUserName(),
				testData.getPassword());
	...
	public void testPIDCPatientRegistration() throws Exception {
		 ...
		 * log("Step 5: Login to Practice Portal"); PracticeLoginPage
		 * practiceLogin = new PracticeLoginPage(driver,
		 * testData.getPracticeURL()); PracticeHomePage pPracticeHomePage =
		 * practiceLogin
		 * .login(testData.getPracticeUserName(),testData.getPracticePassword
		 * ());
		 * 
		 * log("Step 6: Click on Patient Search Link"); PatientSearchPage
		 * pPatientSearchPage= pPracticeHomePage.clickPatientSearchLink();
		 * 
		 * log("Step 7: Set Patient Search Fields");
		 * pPatientSearchPage.searchAllPatientInPatientSearch
		 ...
		 * 
		 * log("Step 9: Click on Patient"); PatientDashboardPage
	...
	public void testEHDCsendCCD() throws Exception {
		 ...
		 * RestUtils.setupHttpGetRequest(processingUrl,
		 * testData.getResponsePath()); if
		 * (RestUtils.isMessageProcessingCompleted(testData.getResponsePath()))
	...
	public void testE2ERxPrescription() throws Exception {
		...
		verifyEquals(myPatientPage.txtMyPatientPage.getText(),
				PortalConstants.MyPatientPage);

		log("Step 4: Click on PrescriptionRenewal Link ");
		NewRxRenewalPage newRxRenewalPage = myPatientPage
				.clickPrescriptionRenewal();
		...
		String medicationName = PortalConstants.MedicationName.toString()
				+ String.valueOf(time);
		log("Medication Name :" + medicationName);

		String rxSMSubject = PortalConstants.RxRenewal_Subject_Tag.toString()
				+ String.valueOf(time);
		log("Perscription Subject :" + rxSMSubject);

		String rxSMBody = IntegrationConstants.QUESTION_MESSAGE.toString() + ""
				+ String.valueOf(time);
		log("Perscription Reply :" + rxSMBody);

		log("Step 7: Verify RxRenewal Confirmation Message");
		PortalUtil.setPortalFrame(driver);
		IHGUtil.waitForElement(driver, 5,
				newRxRenewalPage.renewalConfirmationmessage);
		verifyEquals(newRxRenewalPage.renewalConfirmationmessage.getText(),
				PortalConstants.RenewalConfirmation);
		...
		RestUtils.isMedicationDetailsResponseXMLValid(
				testData.getResponsePath(), medicationName);

		String postXML = RestUtils.findValueOfMedicationNode(
				testData.getResponsePath(), "Medication", medicationName,
				rxSMSubject, rxSMBody, testData.getPrescriptionPath());
	...
	public void testGEAdaptersendCCD() throws Exception {
		...
		/*
		 * log("Step 3: Get processing status until it is completed"); boolean
		 * completed = false; for (int i = 0; i < 3; i++) { // wait 10 seconds
		 * so the message can be processed Thread.sleep(180000);
		 * RestUtils.setupHttpGetRequest(processingUrl,
		 * testData.getResponsePath()); if
		 * (RestUtils.isMessageProcessingCompleted(testData.getResponsePath()))
		 * { completed = true; break; } } verifyTrue(completed,
		...
		MessageCenterInboxPage inboxPage = pMyPatientPage
				.clickViewAllMessagesInMessageCenter();
		assertTrue(inboxPage.isInboxLoaded(), "Inbox failed to load properly.");
	...
	public void testAllScriptImportCCD() throws Exception {
		...
		log("Step 2: Do Message Post Request");
		RestUtils.setupHttpPostRequestExceptOauth(testData.getRestUrl(), ccd,
				testData.getResponsePath(), null);
	...
	public void testE2EFormsExport() throws Exception {
		...
		for (int i = 1; i < 3; i++) {
			...
			MyPatientPage pMyPatientPage = createNewPatientPage
					.BetaSiteCreateAccountPage(testData.getFirstName(),
							testData.getLastName(), email,
							testData.getPhoneNumber(), testData.getDob_Month(),
							testData.getDob_Day(), testData.getDob_Year(),
							testData.getZip(), testData.getSSN(),
							testData.getAddress(), testData.getPassword(),
							testData.getSecretQuestion(), testData.getAnswer(),
			...
			PracticeLoginPage practiceLogin = new PracticeLoginPage(driver,
					testData.getPracticeURL());
			PracticeHomePage pPracticeHomePage = practiceLogin.login(
					testData.getPracticeUsername(),
					testData.getPracticePassword());

			log("Step 7: Click on Patient Search Link");
			PatientSearchPage pPatientSearchPage = pPracticeHomePage
					.clickPatientSearchLink();

			log("Step 8: Set Patient Search Fields");
			pPatientSearchPage.searchForPatientInPatientSearch(firstName,
					lastName);
			...
			PortalLoginPage loginPage = new PortalLoginPage(driver,
					testData.getUrl());
			MyPatientPage myPatientPage = loginPage.login(email,
					testData.getPassword());
			...
			FormWelcomePage pFormWelcomePage = pMyPatientPage
					.clickStartRegistrationButton(driver);

			log("Step 16: Click on Continue button ");
			FormBasicInfoPage pFormBasicInfoPage = pFormWelcomePage
					.clickSaveContinue(FormBasicInfoPage.class);

			log("Step 17: Set Basic Information Form Fields");
			FormEmergencyContactPage pFormEmergencyContactPage = pFormBasicInfoPage
					.setBasicInfoFromField();

			log("Step 18: Set Emergency Contact Form Fields");
			FormCurrentSymptomsPage pFormCurrentSymptomsPage = pFormEmergencyContactPage
			...
			log("Step 23:  Getting forms (CCDs) since timestamp: " + since
					+ " using ccdExchange API");
			RestUtils.setupHttpGetRequest(getURL + "?since=" + since + ",0",
					testData.getResponsePath());
			...
			for (int j = 0; j < i; j++) {
				...
				RestUtils.isPatientAppeared(testData.getResponsePath(),
						patientList.get(0), patientList.get(1),
						patientList.get(2));
	...
	public void testOnlineBillPayment() throws Exception {
		...
		MyPatientPage myPatientPage = loginPage.login(
		...
		RestUtils.oauthSetup(testcasesData.getOAuthKeyStore(),
				testcasesData.getOAuthProperty(),
		...
		String lastTimestamp = RestUtils.setupHttpGetRequest(
				testcasesData.getRestUrl() + "?since=" + timestamp,
		...
		String message = RestUtils.prepareSecureMessage(
				testcasesData.getcommunicationXML(), testcasesData.getFrom(),
				testcasesData.getUserName(), reply_Subject, messageThreadID);

		log("Step 11: Do Message Post AMDC Request");
		String processingUrl = RestUtils.setupHttpPostRequest(
				testcasesData.getCommRestUrl(), message,
				testcasesData.getResponsePath());
		...
		RestUtils.setupHttpGetRequest(processingUrl,
				testcasesData.getResponsePath());
		if (RestUtils.isMessageProcessingCompleted(testcasesData
				.getResponsePath())) {
			completed = true;
		}
		...
		RestUtils.setupHttpGetRequest(testcasesData.getCommRestUrl()
				+ "?since=" + since + ",0", testcasesData.getResponsePath());

		log("Step 19: Validate message reply");
		RestUtils
				.isReplyPresent(testcasesData.getResponsePath(), reply_Subject);

		myPatientPage.logout(driver);

		String postPayload = RestUtils.preparePayment(
				testcasesData.getPaymentPath(), messageThreadID, null,
				IntegrationConstants.BILLPAYMENT);

		log("Step 20: Do a Post and get the message");
		processingUrl = RestUtils.setupHttpPostRequest(
				testcasesData.getRestUrl(), postPayload,
				testcasesData.getResponsePath());
		...
		PracticeHomePage practiceHome = practiceLogin.login(
				testcasesData.getPracticeUserName(),
		...
		RestUtils.setupHttpGetRequest(testcasesData.getRestUrl() + "?since="
				+ lastTimestamp, testcasesData.getResponsePath());
	...
	public void testPayNow() throws Exception {
		...
		NoLoginPaymentPage pNoLoginPaymentPage = new NoLoginPaymentPage(driver,
				testcasesData.getUrl());

		log("Step 2: Fill in payment info and submit");
		pNoLoginPaymentPage.FillNoLoginPaymentPage(
		...
		String lastTimestamp = RestUtils.setupHttpGetRequest(
				testcasesData.getRestUrl() + "=payNowpayment" + "&since="
						+ timestamp, testcasesData.getResponsePath());

		log("Step 9: Verify payment details");
		RestUtils.verifyPayment(testcasesData.getResponsePath(),
				pNoLoginPaymentPage.GetAmountPrize() + ".00",
		...
		String postPayload = RestUtils.preparePayment(
				testcasesData.getPaymentPath(), paymentID,
				pNoLoginPaymentPage.GetAmountPrize() + ".00",
				IntegrationConstants.PAYNOWPAYMENT);

		log("Step 10: Do a Post and get the message");
		String processingUrl = RestUtils.setupHttpPostRequest(
				testcasesData.getRestUrl() + "=payNowpayment", postPayload,
				testcasesData.getResponsePath());
		...
		RestUtils.setupHttpGetRequest(testcasesData.getRestUrl()
	...
	public void testVirtualCardSwiper() throws Exception{
		...
		String lastTimestamp = RestUtils.setupHttpGetRequest(
				testcasesData.getRestUrl() + "=vcsPayment" + "&since="
						+ timestamp, testcasesData.getResponsePath());

		log("Step 8: Verify payment details");
		RestUtils.verifyPayment(testcasesData.getResponsePath(),
				Amount + ".00", IntegrationConstants.SUBMITTED,
		...
		String postPayload = RestUtils.preparePayment(
				testcasesData.getPaymentPath(), paymentID, Amount + ".00",
				IntegrationConstants.VCSPAYMENT);

		log("Step 9: Do a Post and get the message");
		String processingUrl = RestUtils.setupHttpPostRequest(
				testcasesData.getRestUrl() + "=vcsPayment", postPayload,
				testcasesData.getResponsePath());
		...
		practiceLogin = new PracticeLoginPage(driver,
				testcasesData.getPracticeURL());
		practiceHome = practiceLogin.login(testcasesData.getPracticeUserName(),
				testcasesData.getPracticePassword());
		...
		String Status = vcsPageChargeHistory.getBillDetails("$" + Amount
				+ ".00");
		assertNotNull(Status,
				"The submitted csv request was not found in the practice ");
		...
		RestUtils.setupHttpGetRequest(testcasesData.getRestUrl()
				+ "=vcsPayment" + "&since=" + lastTimestamp,
				testcasesData.getResponsePath());
	...
	public void testAMDCSecureMessageFromPractice() throws Exception {
		...
		long timestamp = System.currentTimeMillis();
		String message = RestUtils.prepareSecureMessage(
				testData.getSecureMessagePath(), testData.getIntegrationPracticeID(),
				testData.getUserName(), "Test " + timestamp, null);
	...
	public void testStatementPreference() throws Exception {
		...
		myPreferencePage.setStmtPreference(setPref);
		...
		PracticeLoginPage login = practiceHome.logOut();
		...
		myPreferencePage1.checkStmtPreference(setPref);
