...
public class IntegrationPlatformAcceptanceTests10 {
	...
	public void setConnection() throws IOException {
		...
		assertTrue(
				OauthUtils.oauthSetup1O(testData.getOAuthKeyStore(), testData.getOAuthProperty(),
						testData.getOAuthAppToken(), testData.getOAuthUsername(), testData.getOAuthPassword()),
	...
	public void testPIDC() throws Exception {
		...
		payload = OauthUtils.preparePatient(testData.getCommonPath() + "/patient_Oauth10.xml", practicePatientId,
				firstName, lastName, email, null);

		Log4jUtil.log("Step 2: Do a POST call and get processing status URL");
		processingUrl = OauthUtils.setupHttpPostRequest(testData.getPatientRestURL(), payload,
				testData.getCommonPath() + "/response.xml");

		Log4jUtil.log("Processing status URl:" + processingUrl);
		checkProcessingUrlStatus();

		Log4jUtil.log("Step 3: Do a Get call");
		OauthUtils.setupHttpGetRequest(testData.getPatientRestURL() + "?since=" + sinceTime + ",0",
				testData.getCommonPath() + "/response.xml");
	...
	public void testAMDC() throws Exception {
		...
		payload = OauthUtils.prepareSecureMessage(testData.getCommonPath() + "/secureMessage.xml", testData.getFrom(),
				testData.getUserName(), "Test " + sinceTime, null);

		Log4jUtil.log("Step 2: Do Message Post Request");
		processingUrl = OauthUtils.setupHttpPostRequest(testData.getAMDCRestURL(), payload,
				testData.getCommonPath() + "/response.xml");

		Log4jUtil.log("Step 3: Do a Get call");
		OauthUtils.setupHttpGetRequest(testData.getAMDCRestURL() + "?since=" + epoch + ",0",
				testData.getCommonPath() + "/response.xml");
	...
	public void testAppointment() throws Exception {
		...
		Log4jUtil.log("Step 2: Do Message Post Request");
		processingUrl = OauthUtils.setupHttpPostRequest(testData.getAppointmentRestURL(), payload,
				testData.getCommonPath() + "/response.xml");

		Log4jUtil.log("Processing status URl:" + processingUrl);

		Log4jUtil.log("Step 3: Do a Get call");
		OauthUtils.setupHttpGetRequest(testData.getAppointmentRestURL() + "?since=" + sinceTime + ",0",
				testData.getCommonPath() + "/response.xml");
	...
	public void testPostCCD() throws Exception {
		...
		Log4jUtil.log("Step 2: Do Message Post Request");
		processingUrl = OauthUtils.setupHttpPostRequest(testData.getEHDCRestURL(), payload,
				testData.getCommonPath() + "/response.xml");
	...
	public void testGetReadCommunication() throws Exception {
		...
		OauthUtils.setupHttpGetRequest(testData.getReadCommunicationURL() + "?since=" + sinceTime + ",0",
				testData.getCommonPath() + "/response.xml");
	...
	public void testGetCCDExchangeBatch() throws Exception {
		...
		setConnection();
		Log4jUtil.log("Step 2: Do a Get call");
		OauthUtils.setupHttpGetRequest(testData.getccdExchangeBatch() + "?since=" + sinceTime + ",0",
				testData.getCommonPath() + "/response.xml");
	...
	public void testPrescription() throws Exception {
		...
		Log4jUtil.log("Step 2: Do Message Post Request");
		processingUrl = OauthUtils.setupHttpPostRequest(testData.getPrescriptionRestURL(), payload,
				testData.getCommonPath() + "/response.xml");

		Log4jUtil.log("Processing status URl:" + processingUrl);

		Log4jUtil.log("Step 3: Do a Get call");
		OauthUtils.setupHttpGetRequest(testData.getPrescriptionRestURL() + "?since=" + sinceTime + ",0",
				testData.getCommonPath() + "/response.xml");
	...
	public void testGetEvents() throws Exception {
		...
		Log4jUtil.log("Step 2: Do a Get call");
		OauthUtils.setupHttpGetRequest(testData.getPullEventsURL() + "&sinceTime=" + sixMonthAgo + "&maxEvents=400",
				testData.getCommonPath() + "/response.xml");
	...
	public void testGetPayments() throws Exception {
		...
		setConnection();
		Log4jUtil.log("Step 2: Do a Get call");
		OauthUtils.setupHttpGetRequest(testData.getPaymentURL() + "?sinceTime=" + "1419233246000",
				testData.getCommonPath() + "/response.xml");

	}

	public void checkProcessingUrlStatus()
	...
	public void testGetCcdExchangePdfBatch() throws Exception {
		...
		Log4jUtil.log("Step 2: Do a Get call");
		OauthUtils.setupHttpGetRequest(testData.getCcdExchangePdfBatchURL() + "?sinceTime=" + "1419233246000",
				testData.getCommonPath() + "/response.xml");
	...
	public void testPIDCV2() throws Exception {
		...
		if (IHGUtil.getEnvironmentType().toString().equalsIgnoreCase("DEV3")) {
			postPidcV2RestUrl = postPidcV2RestUrl.replaceAll("https://dev3-int.dev.medfusion.net",
					"http://d3-pp-aapp07.dev.medfusion.net:8080/rwsdk");
		}
		Log4jUtil.log("postPidcV2 " + postPidcV2RestUrl);
		processingUrl = OauthUtils.setupHttpPostRequest(postPidcV2RestUrl, payload,
				testData.getCommonPath() + "/response.xml");
	...
	public void testGetPayNow() throws Exception {
		...
		Log4jUtil.log("Step 2: Do a Get call ");
		OauthUtils.setupHttpGetRequest(testData.getPayments() + "=payNowpayment" + "&since=" + "1419233246000",
				testData.getCommonPath() + "/response.xml");
	...
	public void testPostStatement() throws Exception {
		...
		Log4jUtil.log("Step 2: Do a POST call and get processing status URL ");
		processingUrl = OauthUtils.setupHttpPostRequest(testData.getStatement(), payload,
				testData.getCommonPath() + "/response.xml");
	...
	public void testPostBalance() throws Exception {
		...
		Log4jUtil.log("Step 2: Do a POST call and get processing status URL ");
		processingUrl = OauthUtils.setupHttpPostRequest(testData.getBalance(), payload,
				testData.getCommonPath() + "/response.xml");
	...
	public void testMassMessaging() throws Exception {
		...
		Log4jUtil.log("Step 2: Do Message Post Request");
		processingUrl = OauthUtils.setupHttpPostRequest(testData.getMassAdminMessage(), payload,
				testData.getCommonPath() + "/response.xml");
	...
	public void testDirectSearch() throws Exception {
		...
		Log4jUtil.log("Step 2: Do Message Post Request");
		processingUrl = OauthUtils.setupHttpPostRequest(testData.getDirectorySearch(), payload,
				testData.getCommonPath() + "/response.xml");
	...
	public void testDirectMessage() throws Exception {
		...
		setConnection();
		Log4jUtil.log("Step 2: Do Message Post Request");
		OauthUtils.setupHttpPostRequest(testData.getDirectMessages(), payload,
				testData.getCommonPath() + "/response.xml");
		...
		String msgUid = RestUtils.verifyUnseenMessageListAndGetMessageUID(testData.getCommonPath() + "/response.xml",
				subject);
	...
	public void testPostAppointment() throws Exception {
		...
		Log4jUtil.log("Step 2: Do Message Post Request");
		processingUrl = OauthUtils.setupHttpPostRequest(testData.getAppointmentData(), payload,
				testData.getCommonPath() + "/response.xml");
	...
	public void testStatementPreference() throws Exception {
		...
		Log4jUtil.log("Step 2: Do a POST call and get processing status URL");
		processingUrl = OauthUtils.setupHttpPostRequest(testData.getStatementPreference(), payload,
				testData.getCommonPath() + "/response.xml");

		Log4jUtil.log("Processing status URl:" + processingUrl);
		sinceTime = sinceTime * 1000;
		Log4jUtil.log("Step 3: Do a Get call");
		OauthUtils.setupHttpGetRequest(testData.getStatementPreference() + "?since=" + sinceTime + "",
				testData.getCommonPath() + "/response.xml");
	}
}
