...
public class MU2AcceptanceTest extends BaseTestNGWebDriver {
	...
	public void testMU2PullAPI1() throws Exception {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			ActionTimestamp = UtilityFunctions.FindEventInResonseXML(testData.getResponsePath(), MU2Constants.EVENT, MU2Constants.RESOURCE_TYPE, list.get(i),
					timestamp, intuitPatientID1);
		...
		List<Object> downloadList = IHGUtil.searchResultsSubstring(driver, "//table[@id='activityTable']/tbody",
				new ArrayList<String>(Arrays.asList(MU2Constants.ACCOUNT_ACTIVITY_DOWNLOADED, portalTime.get(1))));
		...
		List<Object> transmitList = IHGUtil.searchResultsSubstring(driver, "//table[@id='activityTable']/tbody",
				new ArrayList<String>(Arrays.asList(MU2Constants.ACCOUNT_ACTIVITY_TRANSMITTED, portalTime.get(2))));
	...
	public void testMU2PullAPI2() throws Exception {
		...
		AppointmentRequestStep3Page apptRequestStep3 = apptRequestStep2.fillInForm(PortalConstants.PreferredTimeFrame, PortalConstants.PreferredDay,
				PortalConstants.ChoosePreferredTime, PortalConstants.ApptReason, PortalConstants.WhichIsMoreImportant, "1234567890");
		...
		ActionTimestamp = UtilityFunctions.FindEventInResonseXML(testData.getResponsePath(), MU2Constants.EVENT, MU2Constants.TRANSMIT_RESOURCE_TYPE,
				MU2Constants.TRANSMIT_ACTION, timestamp, intuitPatientID1);
		...
		ActionTimestamp = UtilityFunctions.FindEventInResonseXML(testData.getResponsePath(), MU2Constants.EVENT, MU2Constants.APPOINTMENT_RESOURCE_TYPE,
				MU2Constants.TRANSMIT_ACTION, timestamp, intuitPatientID1);
		...
		ActionTimestamp = UtilityFunctions.FindEventInResonseXML(testData.getResponsePath(), MU2Constants.EVENT, MU2Constants.PRESCRIPTION_RESOURCE_TYPE,
				MU2Constants.TRANSMIT_ACTION, timestamp, intuitPatientID1);
		...
		ActionTimestamp = UtilityFunctions.FindEventInResonseXML(testData.getResponsePath(), MU2Constants.EVENT, MU2Constants.TRANSMIT_RESOURCE_TYPE,
				MU2Constants.VIEW_ACTION, timestamp, intuitPatientID2);
		...
		ActionTimestamp = UtilityFunctions.FindEventInResonseXML(testData.getResponsePath(), MU2Constants.EVENT, MU2Constants.TRANSMIT_RESOURCE_TYPE,
				MU2Constants.TRANSMIT_ACTION, timestamp, intuitPatientID2);
	...
	public void testMU2PullAPI3() throws Exception {
		...
		actionTimestamp = UtilityFunctions.FindEventInResonseXML(testData.getResponsePath(), MU2Constants.EVENT, MU2Constants.TRANSMIT_RESOURCE_TYPE,
				MU2Constants.VIEW_ACTION, timestamp, intuitPatientID);
		...
		actionTimestamp = UtilityFunctions.FindEventInResonseXML(testData.getResponsePath(), MU2Constants.EVENT, MU2Constants.TRANSMIT_RESOURCE_TYPE,
				MU2Constants.TRANSMIT_ACTION, timestamp, intuitPatientID);
	...
	public void testMU2PullAPI4() throws Exception {
		...
		String ActionTimestamp1 = UtilityFunctions.FindEventInResonseXML(testData.getResponsePath(), MU2Constants.EVENT, MU2Constants.RESOURCE_TYPE,
				MU2Constants.VIEW_ACTION, timestamp, intuitPatientID);
		...
		String ActionTimestamp2 = UtilityFunctions.FindEventInResonseXML(testData.getResponsePath(), MU2Constants.EVENT, MU2Constants.RESOURCE_TYPE,
				MU2Constants.DOWNLOAD_ACTION, timestamp, intuitPatientID);
		...
		List<Object> viewList = IHGUtil.searchResultsSubstring(driver, "//table[@id='activityTable']/tbody",
				new ArrayList<String>(Arrays.asList(MU2Constants.ACCOUNT_ACTIVITY_VIEWED, viewEventTime)));
		if (!viewList.isEmpty()) {
			BaseTestSoftAssert.assertTrue(((Boolean) viewList.get(1)).booleanValue());
		...
		List<Object> downloadList = IHGUtil.searchResultsSubstring(driver, "//table[@id='activityTable']/tbody",
				new ArrayList<String>(Arrays.asList(MU2Constants.ACCOUNT_ACTIVITY_DOWNLOADED, downloadEventTime)));
		if (!downloadList.isEmpty()) {
			BaseTestSoftAssert.assertTrue(((Boolean) downloadList.get(1)).booleanValue());
		} else {
			BaseTestSoftAssert.assertTrue(false, "Health Information Downloaded event not present");
	...
	public void testMU2PushAPI() throws Exception {
		...
		UtilityFunctions.setupHttpGetRequest(
				UtilityFunctions.getURL(MU2Constants.PROTOCOL, testData.getPushHost(), MU2Constants.PORT, testData.getPushAPIURL(), testData.getPushSubjectID()),
				testData.getPushResponsePath());

		log("Step 2: Assert for 'View' event present in Push API response xml for Patient1");
		assertTrue(UtilityFunctions.findPushEventsInResonseXML(testData.getPushResponsePath(), testData.getPatient1FN(), testData.getPatient1LN(),
				MU2Constants.VIEW_ACTION), "Push API event 'View' is not present for Patient1");

		log("Step 3: Assert for 'Download' event present in Push API response xml for Patient1");
		assertTrue(UtilityFunctions.findPushEventsInResonseXML(testData.getPushResponsePath(), testData.getPatient1FN(), testData.getPatient1LN(),
				MU2Constants.DOWNLOAD_ACTION), "Push API event 'Download' is not present for Patient1");

		log("Step 4: Assert for 'Transmit' event present in Push API response xml for Patient1");
		assertTrue(UtilityFunctions.findPushEventsInResonseXML(testData.getPushResponsePath(), testData.getPatient1FN(), testData.getPatient1LN(),
				MU2Constants.TRANSMIT_ACTION), "Push API event 'Transmit' is not present for Patient1");

		log("Step 5: Assert for 'View' event present in Push API response xml for Patient2");
		assertTrue(UtilityFunctions.findPushEventsInResonseXML(testData.getPushResponsePath(), testData.getPatient2FN(), testData.getPatient2LN(),
				MU2Constants.VIEW_ACTION), "Push API event 'View' is not present for Patient2");

		log("Step 6: Assert for 'Download' event present in Push API response xml for Patient2");
		assertTrue(UtilityFunctions.findPushEventsInResonseXML(testData.getPushResponsePath(), testData.getPatient2FN(), testData.getPatient2LN(),
				MU2Constants.DOWNLOAD_ACTION), "Push API event 'Download' is not present for Patient2");

		log("Step 7: Assert for 'Transmit' event present in Push API response xml for Patient2");
		assertTrue(UtilityFunctions.findPushEventsInResonseXML(testData.getPushResponsePath(), testData.getPatient2FN(), testData.getPatient2LN(),
				MU2Constants.TRANSMIT_ACTION), "Push API event 'Transmit' is not present for Patient2");
		 ...
		 * assertTrue(XMLHandler.xmlComparison(testData.getPushResponsePath(),testData.getPushExpectedXMLPath())
		 * ,"Push API response xml and expected xml are not same");
		 * 
		 * 
		 ...
		 * 
		 * log(" Step 1A: Get latest subjectID from DB"); String subjectID =
		 * TrackingDBHandler.getLatetSubjectIDFromDB(MU2Constants.PUSH_API_MSG_TYPE,MU2Constants.PUSH_API_PROCESSING_STATUS_TYPE,testData.getPushPracticeID());//(
		 * MU2Constants.PUSH_API_MSG_TYPE,MU2Constants.PUSH_API_PRACTICEID);//testData.getPushPracticeID());
		 * 
		 * log("Closing DB connection"); TrackingDBHandler.closeConnection();
		 * 
		 * log(" Step 2A: Send Push API HTTP GET Request for the latest SubjectID"); UtilityFunctions.setupHttpGetRequest(UtilityFunctions.getURL(
		 * MU2Constants.PROTOCOL,testData.getPushHost(), MU2Constants.PORT,testData.getPushAPIURL(),subjectID),testData.getPushResponsePath());
		 * 
		 * 
		 * log("Step 3A: Assert for 'View' event present in Push API response xml for Patient1");
		 * assertTrue(UtilityFunctions.findPushEventsInResonseXML(testData.getPushResponsePath(),
		 * testData.getPatient1FN(),testData.getPatient1LN(),MU2Constants.VIEW_ACTION),"Push API event 'View' is not present for Patient1");
		 * 
		 * log("Step 4A: Assert for 'Download' event present in Push API response xml for Patient1");
		 * assertTrue(UtilityFunctions.findPushEventsInResonseXML(testData.getPushResponsePath(),
		 * testData.getPatient1FN(),testData.getPatient1LN(),MU2Constants.DOWNLOAD_ACTION),"Push API event 'Download' is not present for Patient1");
		 * 
		 * log("Step 5A: Assert for 'Transmit' event present in Push API response xml for Patient1");
		 * assertTrue(UtilityFunctions.findPushEventsInResonseXML(testData.getPushResponsePath(),
		 * testData.getPatient1FN(),testData.getPatient1LN(),MU2Constants.TRANSMIT_ACTION),"Push API event 'Transmit' is not present for Patient1");
		 */
