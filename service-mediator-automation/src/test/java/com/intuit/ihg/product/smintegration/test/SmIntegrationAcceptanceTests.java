...
import com.intuit.ihg.product.smintegration.page.AppointmentReqtStep3Page;
import com.intuit.ihg.product.smintegration.page.AppointmentRequestFirstPage;
import com.intuit.ihg.product.smintegration.page.BetaCreateNewPatientPage;
...
public class SmIntegrationAcceptanceTests extends BaseTestNGWebDriver {
	 ...
	 * Click on Appointment Request tab
	 * Create a new appointment
	 * Check the Integration Status id in DB
	 * Check the ppia_status and state
	 * Get ReplyToMessage ID (i.e ppia_id) from the outbound xml and update same in inbound xml
	 * Update patient,Provider and practice details in xml and do inbound
	 * Check the message in patient portal
	 * =============================================================
	...
	public void testApptRequestOutboundAndInbound() throws Exception {
		...
		String appointmentReason = apptRequestStep2.fillInForm(PortalConstants.PreferredTimeFrame,
		...
		{				
			...
			String updatedReqXmlInString = util.updateXML(inputxml,SmIntegrationConstants.TAG_ALLSCRIPT_ID, SmIntegrationConstants.ALLSCRIPT_OLD_VALUE,allScript_New_Value);

			
			log("Updating the xml with Intuit Patient ID");
			updatedReqXmlInString = util.updateXML(updatedReqXmlInString,SmIntegrationConstants.TAG_PATIENT_ID, SmIntegrationConstants.REQUESTORPATIENTID_OLDVALUE,patientID);

			log("###################updatedReqXmlInString"+updatedReqXmlInString);

			log("Updating the xml with Intuit Practice ID");
			updatedReqXmlInString = util.updateXML(updatedReqXmlInString,SmIntegrationConstants.TAG_SITEID, SmIntegrationConstants.SITEID_OLDVALUE,testData.getPracticeID());

			log("Updating the xml with Intuit Provider ID");
			updatedReqXmlInString = util.updateXML(updatedReqXmlInString,SmIntegrationConstants.TAG_PROVIDERID, SmIntegrationConstants.PROVIDERID_OLDVALUE,testData.getIntuitProviderID());

			log("Updating the xml with Intuit Provider ID");
			updatedReqXmlInString = util.updateXML(updatedReqXmlInString,SmIntegrationConstants.TAG_PROVIDERID, SmIntegrationConstants.PROVIDERID_OLDVALUE,testData.getIntuitProviderID());

			log(" Updating the xml with 'Reply to Message ID");
			updatedReqXmlInString = util.updateXML(updatedReqXmlInString,SmIntegrationConstants.TAG_REPLY_TO_MESSAGE_ID, SmIntegrationConstants.REPLY_TO_MESSAGE_VALUE, ppia_id);
			...
			updatedReqXmlInString = util.updateXML(updatedReqXmlInString,SmIntegrationConstants.TAG_SUBJECT, SmIntegrationConstants.SUBJECT_OLD_APPOINTMENT,emailSubject);
			updatedReqXmlInString = util.updateXML(updatedReqXmlInString,SmIntegrationConstants.TAG_EMAILCONTENT, SmIntegrationConstants.APPOINT_EMAIL_OLD_CONTENT,emailContent);
...
//			MyPatientPage home = login.login(testData.getUserName(), testData.getPassword());
	 ...
	 * @Date:-23/Aug/2013
	 * @User Story ID in Rally : test
	 * @StepsToReproduce: List steps : Login to patient portal Click on Create
	 *                    Account button Create a new new patient Check the PGDB
	 *                    Integration ID Status Check ppia_status and state in
	 *                    SM DB Check the ppia_response_xml
	 *                    ======================
	 *                    ====================================== Doesn't work in
	...
	public void testPatientRegistrationOutBoundAndInBound() throws Exception {
		...
		log("step 1: Get Data from Excel");
		SmIntegration smIntegration = new SmIntegration();
		SmIntegrationTestData testcasesData = new SmIntegrationTestData(
				smIntegration);
		SmIntegrationUtil util = new SmIntegrationUtil(driver);
		...
		String email = PortalUtil.createRandomEmailAddress(testcasesData
				.getEmail());
		log("email:-" + email);
		MyPatientPage pMyPatientPage = createNewPatientPage
				.BetaSiteCreateAccountPage(testcasesData.getFirstName(),
						testcasesData.getLastName(), email,
						testcasesData.getPhoneNumber(),
						testcasesData.getDobMonth(), testcasesData.getDobDay(),
						testcasesData.getDobYear(), testcasesData.getZip(),
		...
		String integrationStatusID = SmIntegrationUtil.getintegrationStatusIDFromDB(integrationId, testcasesData.getPGDBName(), testcasesData.getDBEnv(), testcasesData.getDBUserName(), testcasesData.getDBPassword());
			...
			log("Step 12 : Verify whether the ppia status id");
			BaseTestSoftAssert.verifyEquals(dataMap.get(SmIntegrationConstants.PPIA_STATUS), SmIntegrationConstants.PPIASTATUS_VALUE, "Failure***Integration Status ID is not as expected");

			log("Step 13 : Verify whether the ppia state id");
			if(dataMap.get(SmIntegrationConstants.PPIA_STATUS).contains(SmIntegrationConstants.PPIASTATE_VALUE1)&&dataMap.get(SmIntegrationConstants.PPIA_STATE).contains( SmIntegrationConstants.PPIASTATE_VALUE2 ))
			{
				BaseTestSoftAssert.verifyTrue(true);
			}
			else
		...
		log("Path for Registration Inbound XML"
				+ SmIntegrationConstants.PATIENT_REGISTRATION_REQUEST
				+ "Connecthub-Inbound-RegistrationRequestProcessed.xml");

		String inputxmlFilePath = SmIntegrationConstants.PATIENT_REGISTRATION_REQUEST
				+ "Connecthub-Inbound-RegistrationRequestProcessed.xml";
		...
		String updatedReqXmlInString = util.updateXML(inputxml,
				SmIntegrationConstants.TAG_ALLSCRIPTPATIENTID,
				SmIntegrationConstants.ALLSCRIPTPATIENTID_OLDVALUE,
				IHGUtil.createRandomNumericString());

		log("Updating the xml with REQUESTORPATIENTID");
		updatedReqXmlInString = util.updateXML(updatedReqXmlInString,
				SmIntegrationConstants.TAG_REQUESTORPATIENTID,
				SmIntegrationConstants.REQUESTORPATIENTID_OLDVALUE,
				actualMemberID);

		log("Updating the xml with MEMBERID");
		updatedReqXmlInString = util.updateXML(updatedReqXmlInString,
				SmIntegrationConstants.TAG_MEMBERID,
				SmIntegrationConstants.MEMBERID_OLDVALUE, actualMemberID);

		log("Updating the xml with PROVIDERID");
		updatedReqXmlInString = util.updateXML(updatedReqXmlInString,
				SmIntegrationConstants.TAG_PROVIDERID,
				SmIntegrationConstants.PROVIDERID_OLDVALUE,
				testcasesData.getPracticeProviderID());

		log("Updating the xml with Desitination Site ID");
		updatedReqXmlInString = util.updateXML(updatedReqXmlInString,
				SmIntegrationConstants.TAG_SITEID,
				SmIntegrationConstants.SITEID_OLDVALUE,
				testcasesData.getPracticeID());

		log("Request XML #########################\n" + updatedReqXmlInString);

		log("Posting the request xml and getting the reponse");
		String responseXml = util.postXMLRequest(
				testcasesData.getEndPointUrl(), updatedReqXmlInString);

		log("Converting the response string to an xml");
		util.writeStringToXml(
				SmIntegrationConstants.PATIENT_REGISTRATION_RESPONSE
		...
		/*
		 * if(dataMap.get(SmIntegrationConstants.PPIA_STATE).contains(
		 * SmIntegrationConstants
		 * .PPIASTATE_VALUE1)||dataMap.get(SmIntegrationConstants
		 * .PPIA_STATUS).contains( SmIntegrationConstants.PPIASTATE_VALUE2)) {
		 * BaseTestSoftAssert.verifyTrue(true); } else {
		 * verifyTrue(false,"Failure*** State Value is not as expected"); }
		 */
		Thread.sleep(30000);
		...
		String externalPatientRef = SmIntegrationUtil.findAttributeOfChildNode(
				SmIntegrationConstants.PATIENT_REGISTRATION_RESPONSE
						+ "dbresponse.xml", "patientAdminRequest",
				"memberEntity", "externalPatientRef");
		...
		String query = "Select * from externalpracticemembermap where externalid='"
				+ externalPatientRef + "';";
		log("query+++++++++++++++++" + query);
		assertTrue(SmIntegrationUtil.getRowCount(testcasesData.getPGDBName(),
				query, testcasesData.getDBEnv(), testcasesData.getDBUserName(),
	 ...
	 * @Date:-28/Aug/2013
	 * @User Story ID in Rally : test
	 * @StepsToReproduce: List steps : Automation steps listed below Login to
	 *                    PHR Click on 'View All Messages' Check the PGDB
	 *                    Integration ID Status Check the ppia_response_xml
	 *                    ======
	 *                    ======================================================
	 *                    Doesn't work in DEMO,PROD,P10 and QA3 since env is not
	 *                    ready
	 *                    ==================================================
	 *                    ===========
	 * @AreaImpacted :- Description
	 ...
	 */
	@Test(enabled = true, groups = { "AcceptanceTests" }, retryAnalyzer = RetryAnalyzer.class)
	public void testHealthReminderAndReadReceiptInboundAndOutbound()
			throws Exception {
		log("Execution Environment: " + IHGUtil.getEnvironmentType());
		log("Execution Browser: " + TestConfig.getBrowserType());
		...
		log("Path for Health Reminder Inbound XML"
				+ SmIntegrationConstants.PATIENT_HEALTH_REMINDER_REQUEST
				+ "Connecthub-Inbound-HealthReminder.xml");

		String inputxmlFilePath = SmIntegrationConstants.PATIENT_HEALTH_REMINDER_REQUEST
				+ "Connecthub-Inbound-HealthReminder.xml";
		...
		String updatedReqXmlInString = util.updateXML(inputxml,
				SmIntegrationConstants.TAG_MEMBERID,
				SmIntegrationConstants.MEMBERID_OLDVALUE,
				testcasesData.getPatientID());
		...
		if (dataMap.get(SmIntegrationConstants.PPIA_STATUS).contains(
				...
				&& dataMap.get(SmIntegrationConstants.PPIA_STATE).contains(
						SmIntegrationConstants.PPIASTATE_VALUE2)) {
			BaseTestSoftAssert.verifyTrue(true);
		} else {
			verifyTrue(false,
					"Failure*** State and Status Values are not as expected");
		}

		log("Step 5: Write the DB xml to below path");
		util.writeStringToXml(
				SmIntegrationConstants.PATIENT_HEALTH_REMINDER_REQUEST
						+ "inbounddbresponse.xml",
				dataMap.get(SmIntegrationConstants.PPIA_REQUEST_XML));

		log("Step 6: get the staff ID attribure value from the DB response XML");
		String externalStaffID_Db_Response = SmIntegrationUtil
				.findAttributeOfChildNode(
						SmIntegrationConstants.PATIENT_HEALTH_REMINDER_REQUEST
								+ "inbounddbresponse.xml", "generalMessage",
						"provider", "staffRef");
		log("EXTERNAL STAFF ID" + externalStaffID_Db_Response);

		log("Step 7: get Patient ID attribute value from the DB response XML");
		String patientID_Db_Response = SmIntegrationUtil
				.findValueOfGrandChildNode(
						SmIntegrationConstants.PATIENT_HEALTH_REMINDER_REQUEST
								+ "inbounddbresponse.xml", "generalMessage",
						"reference");// , "staffRef");
		log("Patient ID" + patientID_Db_Response);

		verifyEquals(externalStaffID_Db_Response.trim(),
				testcasesData.getIntuitProviderID());
		...
		verifyTrue(pPhrHomePage.isSearchPageLoaded(),
				"Expected the PhrHomePage to be loaded, but it was not.");

		log("step 4:Click on View All Messages link PHR home page");
		PhrMessagesPage pPhrMessagesPage = pPhrHomePage
				.clickOnViewAllMessages();
		...
		util.writeStringToXml(
				SmIntegrationConstants.PATIENT_HEALTH_REMINDER_REQUEST
						+ "outboundbresponse.xml",
				dataMapOutBound.get(SmIntegrationConstants.PPIA_REQUEST_XML));

		log("Step 8: get the externalPatientRef value to query the DB");
		String healthMessageSubject = SmIntegrationUtil.findValueOfChildNode(
				SmIntegrationConstants.PATIENT_HEALTH_REMINDER_REQUEST
						+ "outboundbresponse.xml", "readReceiptMessage",
				"messageSubject");
		String healthMessageBody = SmIntegrationUtil.findValueOfChildNode(
				SmIntegrationConstants.PATIENT_HEALTH_REMINDER_REQUEST
						+ "outboundbresponse.xml", "readReceiptMessage",
				"messageBody");
		...
		verifyEquals(
				healthMessageSubject.trim(),
				SmIntegrationConstants.PATIENTHEALTH_REMINDER_OUTBOUND_MESSAGE_SUBJECT);
		verifyEquals(
				healthMessageBody.trim(),
				SmIntegrationConstants.PATIENTHEALTH_REMINDER_OUTBOUND_MESSAGE_BODY);
	 ...
	 * @Date:-30/Aug/2013
	 * @User Story ID in Rally : test
	 * @StepsToReproduce: List steps : Use Patient Invite XML.xml file to
	 *                    immulte allscripts patient invite message. Update xml
	 *                    with patient/practice/provider details. Use url and
	 *                    post xml to Intuit Query to SM DB and Verify
	 *                    ppia_request_xml, ppia_status & ppia_state values
	 *                    Query to Portal DB and Verify Patient inactive status
	 *                    ======================================================
	 *                    ====== Doesn't work in DEMO,PROD,P10 and QA3 since env
	 *                    is not ready
	 *                    ==========================================
	 *                    ===================
	 * @AreaImpacted :- Description
	...
	public void testPatientInviteInBound() throws Exception {
		...
		log("Password: " + testcasesData.getPassword());
		log("URL: " + testcasesData.getUrl());
		log("Path for Patient Invite Inbound XML"
				+ SmIntegrationConstants.PATIENT_INVITE_REQUEST
				+ "Patient Invite XML.xml");

		String inputxmlFilePath = SmIntegrationConstants.PATIENT_INVITE_REQUEST
				+ "Patient Invite XML.xml";
		...
		String updatedReqXmlInString = util.updateXML(inputxml,
				SmIntegrationConstants.TAG_ALLSCRIPTPATIENTMRN,
				SmIntegrationConstants.ALLSCRIPTPATIENTID_OLDVALUE,
				IHGUtil.createRandomNumericString());
		...
		updatedReqXmlInString = util.updateXML(updatedReqXmlInString,
				SmIntegrationConstants.TAG_FIRSTNAME,
				SmIntegrationConstants.PATIENTFIRSTNAME_OLDVALUE,
				testcasesData.getFirstName());

		log("Updating the xml with LASTNAME");
		updatedReqXmlInString = util.updateXML(updatedReqXmlInString,
				SmIntegrationConstants.TAG_LASTNAME,
				SmIntegrationConstants.PATIENTLASTNAME_OLDVALUE,
				testcasesData.getLastName());

		log("Updating the xml with EMAIL");
		updatedReqXmlInString = util.updateXML(updatedReqXmlInString,
				SmIntegrationConstants.TAG_EMAIL,
				SmIntegrationConstants.PATIENTEMAIL_OLDVALUE,
				testcasesData.getEmail());

		log("Updating the xml with ADDRESS");
		updatedReqXmlInString = util.updateXML(updatedReqXmlInString,
				SmIntegrationConstants.TAG_ADDRESS,
				SmIntegrationConstants.PATIENTADDRESS_OLDVALUE,
				testcasesData.getAddress());

		log("Updating the xml with CITY");
		updatedReqXmlInString = util.updateXML(updatedReqXmlInString,
				SmIntegrationConstants.TAG_CITY,
				SmIntegrationConstants.PATIENTCITY_OLDVALUE,
				testcasesData.getAddressCity());

		log("Updating the xml with STATE");
		updatedReqXmlInString = util.updateXML(updatedReqXmlInString,
				SmIntegrationConstants.TAG_STATE,
				SmIntegrationConstants.PATIENTSTATE_OLDVALUE,
				SmIntegrationConstants.PATIENTSTATE_NEWVALUE);

		log("Updating the xml with POSTALCODE");
		updatedReqXmlInString = util.updateXML(updatedReqXmlInString,
				SmIntegrationConstants.TAG_POSTALCODE,
				SmIntegrationConstants.PATIENTPOSTALCODE_OLDVALUE,
		...
		log("Converting the response string to an xml");
		util.writeStringToXml(SmIntegrationConstants.PATIENT_INVITE_REQUEST
				+ "response.xml", responseXml);
		...
		String query = "select * from memberinactive where emrid='"
				+ externalPatientRef + "';";
		log("query+++++++++++++++++" + query);
		assertTrue(SmIntegrationUtil.getRowCount(testcasesData.getPGDBName(),
				query, testcasesData.getDBEnv(), testcasesData.getDBUserName(),
	 ...
	 * @Date:-2/sep/2013
	 * @User Story ID in Rally : test
	 * @StepsToReproduce: List steps : SM (Service Mediator) Provider Initiated
	 *                    Message (from Allscripts) - INBOUND Use
	 *                    Connecthub-Inbound
	 *                    -GeneralMessage_ProviderInitiated.xml file to immulte
	 *                    allscripts provider initiated message. Update xml with
	 *                    our patient/practice/provider details. For Dev3 use
	 *                    'http://dev3-portal-sm01.mf.qhg.local:8080/service/hub?wsdl'
	 *                    this url and post your xml to Intuit Query Database
	 *                    select * from practice_partner_integration_activity
	 *                    where ppia_ppi_id=3007 order by ppia_id desc limit 5;
	 *                    Verify ppia_request_xml, ppia_status & ppia_state
	 *                    values for our record. Check xml in ppia_request_xml
	 *                    column for our record Once the request is success in
	 *                    SM check Patient inbox for provider initiated message.
	 * @AreaImpacted :- Description
	 * @throws Exception
	 */
	...
	public void testProviderInitiatedMessageFromAllscripts() throws Exception {
		...
		String inputxmlFilePath = SmIntegrationConstants.PROVIDERINITIATEDMESSAGE_REQUEST
				+ "Connecthub-Inbound-GeneralMessage_ProviderInitiated.xml";
		...
		int rndnumber = IHGUtil.createRandomNumber();
		log("Random Number" + rndnumber);
		String emailSubject = SmIntegrationConstants.SUBJECT_NEWVALUE
				+ rndnumber;
		log("Email Subject#######################################"
				+ emailSubject);
		String emailContent = SmIntegrationConstants.EMAILCONTENT_NEWVALUE
				+ rndnumber;
		log("Email Subject#######################################"
				+ emailContent);
		...
		updatedReqXmlInString = util.updateXML(updatedReqXmlInString,
				SmIntegrationConstants.TAG_SUBJECT,
				SmIntegrationConstants.SUBJECT_OLDVALUE, emailSubject);

		log("Updating the xml with EMAILCONTENT");
		updatedReqXmlInString = util.updateXML(updatedReqXmlInString,
				SmIntegrationConstants.TAG_EMAILCONTENT,
				SmIntegrationConstants.EMAILCONTENT_OLDVALUE, emailContent);
		...
		log("Converting the response string to an xml");
		util.writeStringToXml(
				SmIntegrationConstants.PROVIDERINITIATEDMESSAGE_RESPONSE
						+ "response.xml", responseXml);
		...
		log("Step 4 : get the externalPatientRef value to query the DB");
		String emailSubject_Db_Response = SmIntegrationUtil
				.findValueOfChildNode(
						SmIntegrationConstants.PROVIDERINITIATEDMESSAGE_RESPONSE
								+ "dbresponse.xml", "generalMessage",
						"messageSubject");
		String emailContent_Db_Response = SmIntegrationUtil
				.findValueOfChildNode(
						SmIntegrationConstants.PROVIDERINITIATEDMESSAGE_RESPONSE
								+ "dbresponse.xml", "generalMessage",
						"messageBody");

		log("emailSubject from Db_Response####################################"
				+ emailSubject_Db_Response);
		log("emailContent from Db_Response####################################"
				+ emailContent_Db_Response);
		...
		log("step 18: login to patient portal (again)");
		PortalLoginPage login = new PortalLoginPage(driver,
				testcasesData.getUrl());
		MyPatientPage home = login.login(testcasesData.getUserName(),
				testcasesData.getPassword());
		...
		log("step 21: Validate message loads and is the right message");
		String actualSubject = message.getPracticeReplyMessageTitle();
		assertTrue(message.getPracticeReplyMessageTitle().trim()
				.equalsIgnoreCase(emailSubject_Db_Response),
				"Expected subject containting [" + emailSubject_Db_Response
