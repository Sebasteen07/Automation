...
public class FormsAcceptanceTests extends BaseTestNGWebDriver {
	...
	public void formsConfigSmokeTest() throws Exception {
		...
		DiscreteFormsList formsPage = sgSteps.logInUserToSG(driver, testData.getProperty("sitegen.username1"),
				EncryptionUtils.decrypt(testData.getProperty("sitegen.password1"))).clickLnkDiscreteForms();
	...
	public void testFormExportImport() throws Exception {
		...
		String automationPracticeID = String
				.valueOf(Utils.getPracticeIDFromPIUrl(testData.getProperty("portal2.url1")));
		SiteGenHomePage sHomePage = new SiteGenLoginPage(driver, testData.getProperty("sitegen.url"))
				.clickOnLoginAsInternalEmployee();
		// now you have to LOG IN MANUALLY AS SUPERUSER, the test will continue after
		// that
		...
		Path exportedFilePath = Paths
				.get(System.getProperty("user.dir") + "\\" + SitegenConstants.FORM_EXPORT_IMPORT + ".txt");
		String exportedFileString = stripVariablesFromExportedForm(
				Files.readAllLines(exportedFilePath, Charsets.UTF_8).get(0));
		String representativeFileString = stripVariablesFromExportedForm(Files.readAllLines(
				Paths.get(ClassLoader.getSystemResource(SitegenConstants.FORM_EXPORT_IMPORT + ".txt").toURI()),
				Charsets.UTF_8).get(0));
		...
		assertFalse(pManageDiscreteForms.openUnpublishedFormPreview(SitegenConstants.FORM_EXPORT_IMPORT)
				.getMessageText().isEmpty());
	...
	private void testEGQSitegenEnabledFlow(DiscreteFormsList formsList, WelcomeScreenPage welcomePage)
			throws Exception {
	...
	private HealthFormListPage testEGQEnabledPIFlow(WebDriver driver) throws Exception {
		...
		HealthFormListPage formsList = Utils.createAndLoginPatientPI(driver, testData, PracticeType.SECONDARY)
				.clickOnHealthForms();
	...
	private void testEGQSitegenDisabledFlow(DiscreteFormsList formsList) throws Exception {
		IHGUtil.PrintMethodName();
		BasicInformationAboutYouPage basicInfoPage = formsList.openDiscreteForm(SitegenConstants.FORM_EGQ_NAME)
				.clicklnkBasicInfoAboutYourPage();
	...
	private String stripVariablesFromExportedForm(String s) {
		return s.replaceAll("\"\\w+Date\":\\d+", "").replaceAll("\"formId\":\"[^\"]+\"", "")
				.replaceAll("\"copiedFrom\":\"[^\"]+\"", "").replaceAll("\"\\w+racticeId\":\\d+", "")
				.replaceAll("\"environment\":\"\\w+\"", "");
	}
}
